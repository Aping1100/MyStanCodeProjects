...
def search_names(name_data, target):
    ...
    for name in name_data:
        # 在search所有name與target皆以大寫搜尋
        up_name = name.upper()
        up_target = target.upper()
        # 將name與target的字數相減加一，就會是搜尋的次數
        for i in range(len(up_name)-len(up_target)+1):
            if up_target == up_name[i:i+len(target)]:
                names.append(name)
    return names
